"""
Conversation to Knowledge — Synor pipeline example.
...
import synor as syn
from synor.connectors import localfs, surrealdb
...
async def _resolve_entities(
    ...
    result = await resolve_entities(
        ...
        resolve_pair=LlmPairResolver(
            model=syn.use_context(RESOLUTION_LLM_MODEL),
        ),
    )
...
async def synor_lifespan(
    ...
    builder.provide(
        ...
        surrealdb.ConnectionFactory(
            ...
            namespace=os.environ.get("SURREALDB_NS", "synor"),
...
app = syn.App(
    syn.AppConfig(name="ConversationToKnowledge"),
    app_main,
)
