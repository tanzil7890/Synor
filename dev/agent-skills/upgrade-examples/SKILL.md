...
description: This skill should be used when upgrading the synor package version in all example pyproject.toml files. It uses regex-based sed commands to efficiently update version constraints across multiple files at once.
---

# Upgrade Example Dependencies

Upgrade the synor version in all example `pyproject.toml` files.
...
```bash
find examples -name "pyproject.toml" -not -path "*/.venv/*" -exec grep -l "synor" {} \; | xargs sed -i '' 's/synor\([^>]*\)>=[0-9][0-9a-zA-Z.]*/synor\1>=VERSION/g'
```

3. Verify the changes show the correct version:

```bash
grep -r "synor.*>=" examples --include="pyproject.toml" | grep -v ".venv"
```
...
git commit -m "chore: upgrade examples deps to synor-VERSION"
...
```bash
gh pr create --base v1 --title "chore: upgrade examples deps to synor-VERSION" --body ""
```
