#!/usr/bin/env python3
"""
Script to generate CLI documentation from Synor Click commands.

This script uses md-click as the foundation but generates enhanced markdown
documentation that's suitable for inclusion in the Synor documentation site.
"""
...
import re
import click
from synor.cli import cli

# Add the synor python directory to the path
...
def clean_usage_line(usage: str) -> str:
    ...
    # Replace 'cli' with 'synor' in usage lines and remove 'Usage:' prefix
    cleaned = usage.replace("Usage: cli ", "synor ")
    # Handle case where it might be "Usage: synor" already
    if cleaned.startswith("Usage: synor "):
        cleaned = cleaned.replace("Usage: synor ", "synor ")
    return cleaned
...
def format_commands_section(help_text: str) -> str:
    ...
    for line in commands_lines:
        ...
        match = re.match(r"^  ([A-Za-z0-9][A-Za-z0-9_-]*)\s{2,}(.+)$", line)
        if match:
...
def _iter_commands(
    ...
    """Yield every command with its fully qualified CLI path.

    Click groups can be nested. Keeping the real parent context produces the
    correct ``synor revocations list`` usage while the qualified heading avoids
    duplicate anchors for common names such as ``list`` and ``show``.
    """

    for name, sub_cmd in sorted(group.commands.items()):
        command_path = (*path, name)
        sub_ctx = click.core.Context(sub_cmd, info_name=name, parent=parent_ctx)
        yield command_path, sub_cmd, sub_ctx
        if isinstance(sub_cmd, click.Group):
            yield from _iter_commands(sub_cmd, sub_ctx, command_path)


def generate_command_docs(cmd: click.Group) -> str:
    """Generate markdown documentation for all commands and nested groups."""

    markdown_content = []

    ctx = click.core.Context(cmd, info_name=cmd.name)
    # Generate only the command details section (remove redundant headers).
    for command_path, sub_cmd, sub_ctx in _iter_commands(cmd, ctx):
...
def main() -> None:
    ...
    print("Generating Synor CLI documentation...")
