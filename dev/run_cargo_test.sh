...
# Users can override with SYNOR_PYTHON if they want a different interpreter.
if [[ -n "${SYNOR_PYTHON:-}" ]]; then
  PY="$SYNOR_PYTHON"
  ...
  echo "hint: create/activate a venv (.venv) or set SYNOR_PYTHON=/path/to/python" >&2
  exit 1
fi
...
cargo check -p synor --features bench-support --benches
