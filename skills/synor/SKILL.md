...
The `@syn.fn` decorator marks functions as Synor processing functions. Add `memo=True` to skip re-execution when inputs/code are unchanged:
...
await syn.mount(setup_fn, arg1)

# Dependent component (blocks until result returned)
result = await syn.use_mount(init_fn)

# Explicit subpath (when you need a specific path, e.g. in loops)
await syn.mount(syn.component_subpath("item", item_id), process_item, item)
```
...
await syn.mount_each(process_message, items, target)
```
...
@syn.fn(memo=True)  # Memo avoids re-calling LLM
...
### 1. Use `@syn.fn` on All Processing Functions

Every function that participates in the pipeline (declares target states, calls mount APIs, etc.) must be decorated with `@syn.fn`.

### 2. Add Memoization for Expensive Operations

```python
@syn.fn(memo=True)  # Skip re-execution when inputs/code unchanged
...
syn.component_subpath("file", str(file.file_path.path))
syn.component_subpath("record", record.id)

# Bad: Unstable identifiers
syn.component_subpath("file", file)      # Object reference
syn.component_subpath("idx", idx)        # Index changes
```
...
|---|---|
| `@synor.flow_def`, `FlowBuilder`, `Flow`, `open_flow` | `syn.App` + a `@syn.fn` main function |
...
| `synor.targets.Postgres`, `synor.targets.*` / `storages.*` | connector targets, e.g. `postgres.declare_table_target(...)` |
| `transform_flow`, `synor.op.function()` | plain `@syn.fn` functions |
