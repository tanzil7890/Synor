# Synor Connectors Reference

Comprehensive reference for all Synor connectors.
...
Target connectors take a `ContextKey` (not the connection object directly) as their first argument. This enables Synor to track the identity of the target across runs.
...
**Import**: `from synor.connectors import postgres`
...
**Import**: `from synor.connectors import sqlite`
...
SQLITE_DB = syn.ContextKey[sqlite.ManagedConnection]("sqlite_db")
...
**Import**: `from synor.connectors import lancedb`
...
**Import**: `from synor.connectors import qdrant`
...
QDRANT_DB = syn.ContextKey[QdrantClient]("qdrant_db")
...
**Import**: `from synor.connectors import surrealdb`
...
```python
from synor.connectors import surrealdb

SURREAL_DB = syn.ContextKey[surrealdb.ConnectionFactory]("surreal_db")
...
**Import**: `from synor.connectors import neo4j`
...
NEO4J_DB = syn.ContextKey[neo4j.ConnectionFactory]("neo4j_db")
...
**Import**: `from synor.connectors import falkordb`
...
FALKOR_DB = syn.ContextKey[falkordb.ConnectionFactory]("falkor_db")
...
**Import**: `from synor.connectors import localfs`
...
**Import**: `from synor.connectors import amazon_s3`
...
```python
from synor.connectors import amazon_s3
...
**Import**: `from synor.connectors import kafka`
...
**Import**: `from synor.connectors import doris`
...
```python
from synor.connectors import doris

DORIS_DB = syn.ContextKey[doris.ManagedConnection]("doris_db")
...
**Import**: `from synor.connectors import google_drive`
...
immutable Drive file ID, separates display paths from identity, resolves
paginated permissions, fences full snapshots with the Changes API, and delays
cursor commits until downstream readiness:
...
source = google_drive.GovernedGoogleDriveSource(
    ...
    connector_instance_id="drive-reader-prod",
    source_scope_id="approved-roots-v1",
    tenant_id="tenant-a",
    group_graph_revision="directory-sync-v42",
)

snapshot = await source.open_governed_snapshot()
await handle_governed_items(snapshot.items())
await snapshot.commit_after_downstream_ready()
```

Run and commit one complete snapshot before opening a change session. Partial
snapshots, rejected cursors, shared-drive authority changes, and policy/group
revision changes require a new complete snapshot. Bare tombstones and Drive
404 responses remain ambiguous rather than being asserted as physical
deletion. Same-corpus event order is authoritative; cross-corpus live state can
prove a move. Folder revocation fences descendants that still depend on the
inactive ancestor, and newly in-scope folders are recursively enumerated even
when a child event arrives first. Permission-expiry retries retain stable
observation identity. The connector produces source observations and
checkpoints; the controlled runtime/certified target owns derivative
