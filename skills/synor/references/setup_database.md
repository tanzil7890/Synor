...
Setup instructions for databases used with Synor.
...
services:
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U synor"]
      interval: 5s
      timeout: 5s
      retries: 5
...
Connection URL: `postgres://synor:synor@localhost:5432/synor`
...
```bash
# .env
POSTGRES_URL=postgres://synor:synor@localhost:5432/synor
```
...
docker logs synor-qdrant      # Check logs
```
