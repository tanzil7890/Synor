...
Environment variables (all required for tests to run):
    ...
    DORIS_DATABASE    - Test database         (default: synor_test)
"""
...
DORIS_DB_KEY: syn.ContextKey[Any] = syn.ContextKey("test_doris_target_db")
...
def _doris_config() -> "doris.DorisConnectionConfig":
    ...
    return doris.DorisConnectionConfig(
        ...
        database=os.getenv("DORIS_DATABASE", "synor_test"),
...
def test_create_table_and_insert_rows(
    ...
    """Test creating a table and inserting rows via syn.App."""
    ...
    app = syn.App(
        syn.AppConfig(name="test_doris_create", environment=synor_env),
...
def test_update_row(
    ...
    synor_env.context_provider.provide(DORIS_DB_KEY, managed_conn)
    app = syn.App(
        syn.AppConfig(name="test_doris_update", environment=synor_env),
        _declare_table_and_rows,
    )
...
def test_delete_row(
    ...
    synor_env.context_provider.provide(DORIS_DB_KEY, managed_conn)
    app = syn.App(
        syn.AppConfig(name="test_doris_delete", environment=synor_env),
        _declare_table_and_rows,
    )
...
def test_no_change_optimization(
    ...
    synor_env.context_provider.provide(DORIS_DB_KEY, managed_conn)
    app = syn.App(
        syn.AppConfig(name="test_doris_noop", environment=synor_env),
        _declare_table_and_rows,
    )
