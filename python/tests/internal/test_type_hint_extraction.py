...
from synor._internal.serde import (
    get_param_annotation,
    serialize,
    unwrap_element_type,
)
from synor._internal.memo_fingerprint import _make_state_deserialize_fn
...
def test_extract_state_type_from_memo_state() -> None:
    """_make_state_deserialize_fn extracts the prev_state type from __synor_memo_state__."""

    class WithMemoState:
        def __synor_memo_state__(self, prev: MyState) -> MemoStateOutcome:
            return MemoStateOutcome(state=prev, memo_valid=True)

    deser = _make_state_deserialize_fn(WithMemoState.__synor_memo_state__)
