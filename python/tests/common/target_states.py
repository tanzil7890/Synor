...
class DictTargetStateStore:
    ...
    def __init__(self, use_async: bool = False) -> None:
        ...
        self._lock = threading.Lock()
        self._action_sink = (
            syn.TargetActionSink.from_async_fn(self._async_sink)
            if use_async
            else syn.TargetActionSink.from_fn(self._sink)
        )
...
class DictsTargetStateStore:
    ...
    def _sink(
        ...
        /,
    ) -> list[syn.ChildTargetDef[DictTargetStateStore] | None]:
        child_state_defs: list[syn.ChildTargetDef[DictTargetStateStore] | None] = []
        if self.sink_exception:
            raise ValueError("injected sink exception")
        with self._lock:
            for name, exists, action, destructive in actions:
                ...
                if exists:
                    child_state_defs.append(syn.ChildTargetDef(self._stores[name]))
                else:
...
class DictsTarget:
    ...
    @staticmethod
    def dict_target(name: str) -> syn.TargetState[DictTargetStateStore]:
...
class AttachmentDictsTargetStateStore:
    ...
    def __init__(
        ...
        self._action_sink = syn.TargetActionSink.from_fn(self._sink)
    ...
    def _sink(
        ...
        actions: Collection[_DictTargetStateStoreAction],
        /,
    ) -> list[syn.ChildTargetDef[_AttachmentChildHandler] | None]:
        child_state_defs: list[syn.ChildTargetDef[_AttachmentChildHandler] | None] = []
        with self._lock:
            for name, exists, action, destructive in actions:
                if action == "insert":
                ...
                if exists:
                    child_state_defs.append(syn.ChildTargetDef(self._handlers[name]))
                else:
                    child_state_defs.append(None)
