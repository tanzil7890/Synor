...
def test_full_reprocess_force_execution_of_memoized_functions() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_full_reprocess_force_execution", environment=synor_env
        ),
        _declare_dict_data,
    )
...
def test_full_reprocess_force_rewrite_unchanged_targets() -> None:
    ...
    app = syn.App(
        syn.AppConfig(name="test_full_reprocess_force_rewrite", environment=synor_env),
        _declare_dict_data,
    )
...
def test_full_reprocess_deleted_targets_not_resurrected() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_full_reprocess_deleted_targets", environment=synor_env
        ),
        _declare_dict_data,
    )
...
def test_full_reprocess_component_memoization() -> None:
    ...
    data_version = 1
    app = syn.App(
        syn.AppConfig(name="test_full_reprocess_component_memo", environment=synor_env),
