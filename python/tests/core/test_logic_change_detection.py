...
def _unload_module_functions(mod: ModuleType) -> None:
    """Unregister logic fingerprints for all Synor functions in a module."""
...
def test_fn_memo_invalidated_on_logic_change() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_fn_memo_invalidated_on_logic_change", environment=synor_env
        ),
        app_main,
    )
...
def test_component_memo_invalidated_on_logic_change() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_component_memo_invalidated_on_logic_change",
...
def test_transitive_fn_memo_bar_memo_changes() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_transitive_fn_memo_bar_memo_changes", environment=synor_env
        ),
        app_main,
    )
...
def test_transitive_fn_memo_bar_plain_changes() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_transitive_fn_memo_bar_plain_changes", environment=synor_env
        ),
        app_main,
    )
...
def test_transitive_component_calls_bar_memo() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_transitive_component_calls_bar_memo", environment=synor_env
        ),
        app_main,
    )
...
def test_transitive_component_mounts_bar_comp_plain() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_transitive_component_mounts_bar_comp_plain",
            environment=synor_env,
        ),
        app_main,
    )
...
def test_transitive_component_mounts_bar_comp_memo() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_transitive_component_mounts_bar_comp_memo", environment=synor_env
        ),
        app_main,
    )
...
def test_fn_memo_invalidated_on_version_bump() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_fn_memo_invalidated_on_version_bump", environment=synor_env
        ),
        app_main,
    )
...
_CHANGE_DETECTED_KEY_J3 = syn.ContextKey[str](
    "_test_logic_tracking_none_ctx_j3", detect_change=True
)


def _create_env_with_ctx(db_name: str, value: str) -> syn.Environment:
...
def test_none_mode_still_detects_change_on_context_key_deps() -> None:
    ...
    app1 = syn.App(syn.AppConfig(name=db_name, environment=env1), app_main)
    ...
    app2 = syn.App(syn.AppConfig(name=db_name, environment=env2), app_main)
...
def test_bound_method_fn_memo_invalidated_on_logic_change() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_bound_method_fn_memo_logic_change", environment=synor_env
        ),
        app_main,
    )
...
def test_bound_method_component_memo_invalidated_on_logic_change() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_bound_method_component_memo_logic_change", environment=synor_env
        ),
        app_main,
    )
...
def test_fn_memo_invalidated_on_deps_change() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_fn_memo_invalidated_on_deps_change", environment=synor_env
        ),
        app_main,
    )
...
def test_deps_fingerprint_contract() -> None:
    ...
    different deps differ, ``__synor_memo_key__()`` is honored, and ``deps``
    composes correctly with ``version``.
    """
    from synor._internal.function import SyncFunction
    ...
    try:
        ...
        # __synor_memo_key__ contract: the transient field must not affect the fp.
        class WithKey:
