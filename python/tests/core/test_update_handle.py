...
async def test_handle_result() -> None:
    app = syn.App(
        syn.AppConfig(name="test_handle_result", environment=synor_env),
        _trivial_fn,
        "hello",
    )
    handle = app.update()
...
async def test_handle_stats_after_completion() -> None:
    ...
    app = syn.App(
        syn.AppConfig(name="test_handle_stats_after_completion", environment=synor_env),
        _process_items,
    )
    handle = app.update()
    await handle.result()
...
async def test_watch_yields_running_then_done() -> None:
    ...
    app = syn.App(
        syn.AppConfig(
            name="test_watch_yields_running_then_done", environment=synor_env
        ),
        _process_items,
    )
    handle = app.update()
    ...
    assert snapshots[-1].status == syn.UpdateStatus.READY
...
async def test_watch_raises_on_error() -> None:
    app = syn.App(
        syn.AppConfig(name="test_watch_raises_on_error", environment=synor_env),
        _failing_main,
    )
    handle = app.update()
...
async def test_watch_with_throttle() -> None:
    ...
    app = syn.App(
        syn.AppConfig(name="test_watch_with_throttle", environment=synor_env),
        _process_items,
    )
    handle = app.update()
    snapshots = []
    async for snapshot in handle.watch():
        snapshots.append(snapshot)
        if snapshot.status == syn.UpdateStatus.RUNNING:
...
async def test_show_progress_with_watch() -> None:
    ...
    app = syn.App(
        syn.AppConfig(name="test_show_progress_with_watch", environment=synor_env),
        _process_items,
    )
    handle = app.update()
    snapshots = []
