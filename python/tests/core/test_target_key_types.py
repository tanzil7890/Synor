...
def test_valid_stable_keys() -> None:
    synor_env = common.create_test_env("test_valid_stable_keys")
    AnyKeyTarget.store.clear()

    app = syn.App(
        syn.AppConfig(name="test_valid_stable_keys", environment=synor_env),
        lambda: None,
    )
    ...
    app = syn.App(
        syn.AppConfig(name="test_valid_stable_keys_run", environment=synor_env),
        declare_keys,
    )
    app.update_blocking()
...
def test_invalid_keys() -> None:
    synor_env = common.create_test_env("test_invalid_keys")
    AnyKeyTarget.store.clear()
...
def test_nested_container_keys() -> None:
    synor_env = common.create_test_env("test_nested_container_keys")
    AnyKeyTarget.store.clear()
    key_input = [1, 2]
    key_expected = (1, 2)
