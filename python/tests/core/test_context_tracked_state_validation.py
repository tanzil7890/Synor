...
- `memo_validation` (`__synor_memo_state__` state functions that decide memo reuse)

When a change-detected context value (or an object reachable inside its canonical form)
exposes `__synor_memo_state__`, it should behave like an argument-borne state
...
Each test creates one :class:`syn.Environment` and simulates cross-run state
...
EMBEDDER_KEY = syn.ContextKey[StatefulEmbedder](
    "_test_ctx_tracked_embedder", detect_change=True
)


def _make_env(db_name: str, embedder: StatefulEmbedder) -> syn.Environment:
...
def test_detect_change_context_state_validation_function_level() -> None:
    ...
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_state_fn", environment=env), _run_embed
    )
    ...
    #        → __synor_memo_state__ returns memo_valid=False → re-execute.
...
TWO_LEVEL_KEY = syn.ContextKey[TwoLevelStatefulEmbedder](
...
def test_detect_change_context_state_valid_with_updated_state() -> None:
    ...
    env = syn.Environment(settings, context_provider=ctx)
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_two_level", environment=env),
...
REPLACE_KEY = syn.ContextKey[StatefulEmbedder](
...
def test_detect_change_context_value_replaced() -> None:
    ...
    env = syn.Environment(settings, context_provider=ctx)
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_replaced", environment=env),
        _run_embed_replace,
    )
...
COMPOSITE_KEY = syn.ContextKey[tuple[str, _Inner]](
...
def test_detect_change_context_composite_state() -> None:
    ...
    env = syn.Environment(settings, context_provider=ctx)
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_composite", environment=env),
...
COMP_KEY = syn.ContextKey[StatefulEmbedder](
...
def test_detect_change_context_state_validation_component_level() -> None:
    ...
    env = syn.Environment(settings, context_provider=ctx)
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_state_comp", environment=env),
        _mount_embed,
    )
...
KEY_A = syn.ContextKey[StatefulEmbedder](
    "_test_ctx_tracked_branching_a", detect_change=True
)
KEY_B = syn.ContextKey[StatefulEmbedder](
...
def test_cache_hit_not_reusable_refreshes_context_fps() -> None:
    ...
    env = syn.Environment(settings, context_provider=ctx)
    app = syn.App(
        syn.AppConfig(name="test_ctx_tracked_branching", environment=env),
        _run_branching,
    )
