...
This test verifies that Synor's environment setup works correctly when
...
See BUG_REPORT/SYNOR_FIX_README.md for detailed background on the bug fix.
"""
...
async def test_async_app_with_fresh_event_loop_first() -> None:
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=ignore_cleanup) as tmpdir:
        ...
        app = syn.App(
            syn.AppConfig(name="test1", environment=env),
            main,
        )
...
async def test_async_app_with_fresh_event_loop_second() -> None:
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=ignore_cleanup) as tmpdir:
        ...
        app = syn.App(
            syn.AppConfig(name="test2", environment=env),
            main,
        )
...
async def test_async_app_with_fresh_event_loop_third() -> None:
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=ignore_cleanup) as tmpdir:
        ...
        app = syn.App(
            syn.AppConfig(name="test3", environment=env),
            main,
        )
...
async def test_multiple_sequential_app_updates() -> None:
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=ignore_cleanup) as tmpdir:
        ...
        env = syn.Environment(syn.Settings(db_path=db_path), name="seq_env")
        ...
        for i in range(3):
            ...
            app = syn.App(
                syn.AppConfig(name=f"seq_test_{i}", environment=env),
                main,
            )
            result = await app.update()
