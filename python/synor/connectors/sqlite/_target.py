...
from synor._internal.rwlock import RWLock
import msgspec
...
class SqliteType(NamedTuple):
    ...
    from synor.connectors.sqlite import SqliteType
...
def _apply_table_actions(
    ...
    outputs: list[syn.ChildTargetDef[_RowHandler] | None] = [None] * len(actions_list)
...
_table_provider = syn.register_root_target_states_provider(
    "synor/sqlite/table", _TableHandler()
)
...
def table_target(
    ...
    *,
    ...
    if isinstance(virtual_table_def, Vec0TableDef):
        managed_conn = syn.use_context(db)
...
def connect(
    ...
    **kwargs: Any,
    ...
    """
    Create a SQLite connection with common defaults for Synor.
