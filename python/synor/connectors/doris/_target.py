"""
Apache Doris target connector for Synor (v1 API).
...
def _generate_stream_load_label() -> str:
    return f"synor_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}"
...
_table_provider = syn.register_root_target_states_provider(
    "synor/doris/table", _TableHandler()
)
...
class DorisTableTarget(
    Generic[RowT, syn.MaybePendingS], syn.ResolvesTo["DorisTableTarget[RowT]"]
):
