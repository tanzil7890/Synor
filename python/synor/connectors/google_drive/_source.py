"""Compatibility Google Drive source utilities.

This module preserves the original path-keyed, read-only ingestion API. The
additive stable-ID and ACL-aware API lives in ``_governed_source``.
"""
...
def _require_google_dependencies() -> None:
    ...
    raise ImportError(
        ...
        "Please install synor[google_drive]."
...
class DriveFile(file.FileLike[str]):
    ...
    def __init__(self, service: Any, info: DriveFileInfo) -> None:
        file_path = DriveFilePath(
            info.display_path or info.name,
    ...
    def file_id(self) -> str:
        """Return the immutable Google Drive file ID."""

        return self._file_id

    @property
    def mime_type(self) -> str:
        """Return the Google Drive MIME type."""
...
def _build_service(
    ...
    *,
    ...
    scopes: Sequence[str] = (_DRIVE_SCOPE,),
) -> Any:
    _require_google_dependencies()
