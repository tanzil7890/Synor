...
def canonical_module_name(obj: typing.Any) -> str:
    ...
    * ``synor update main.py`` -> ``main`` (basename, set by the loader)
    * ``synor update main``    -> ``main`` (importlib's module name)
...
class NotMemoKeyable:
    ...
    def __synor_memo_key__(self) -> typing.NoReturn:
        raise TypeError(
...
def _canonicalize(
    ...
    hook = getattr(obj, "__synor_memo_key__", None)
    if hook is not None and callable(hook):
        k = hook()
        typ = type(obj)
        tag = "hook"
        state_hook = getattr(obj, "__synor_memo_state__", None)
        if state_hook is not None and callable(state_hook):
            tag = "shook"
            # raw function for type hint extraction (unbound method on class)
            raw_fn = getattr(typ, "__synor_memo_state__")
    ...
    try:
        ...
        raise TypeError(
            ...
            "Provide __synor_memo_key__() or register a memo key function."
        ) from None
...
def fingerprint_call(
    ...
    *,
    ...
    Returns a `synor._internal.core.Fingerprint` object (Python wrapper around a
