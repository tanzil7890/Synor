...
// *asterisks* mark the italic Spark accent. See consts.titleMarkup.
//
...
export const CATEGORY_META: Record<Category, { label: string; em?: string; lead: string; thumbClass: string }> = {
  ...
  ingest: { label: 'Local & ', em: 'Connected Data', lead: 'Observe files, rows, or events and reconcile the outcomes each keyed work unit owns.', thumbClass: 'ingest' },
  llm: { label: 'Structured ', em: 'Extraction', lead: 'Extract typed records from code and documents with instructor, BAML, or DSPy.', thumbClass: 'llm' },
  ...
  image: { label: 'Multimodal ', em: 'Search', lead: 'Search images, PDFs, slides, and faces by meaning — the same vector index, a different encoder.', thumbClass: 'image' },
};

export type ExampleCard = {
  slug: string;                      // becomes /docs/examples/<slug>
  title: string;                     // asterisks produce italic Spark
...
export const examples: ExampleCard[] = [
  {
    ...
    index: '00 / 33',
    category: 'ingest',
    thumbLabel: 'Markdown · local JSON',
    description: 'Turn Markdown notes into JSON records, run twice, then edit one note and watch one outcome refresh. No service or network request.',
    tags: [
      { kind: 'src', label: 'Local FS' },
      { kind: 'tgt', label: 'Local FS' },
      { kind: 'ops', label: 'Standard library' },
      { kind: 'lvl', label: 'First run' },
    ],
    footMeta: '~5 min · local',
    sourceSlug: 'local_note_catalog',
    featured: true,
  },
  {
    slug: 'text-embedding',
  ...
  },
  {
    ...
    description: "The smallest end-to-end Synor pipeline \u2014 watch a folder of Markdown, render each file to HTML, and write the outputs to a local folder incrementally.",
    tags: [
...
// Local fallback art: the listing does not depend on an unowned image host.
export const exampleCardImg = (_slug: string): string =>
  `${import.meta.env.BASE_URL}images/synor-mark.svg`;
...
export const POPULAR: Array<{ slug: string; label: string; count: string }> = [
  { slug: 'local-note-catalog', label: 'Run the Local Note Catalog', count: 'Start' },
...
// Full catalog of runnable examples in the synor monorepo, used to build the
...
export type ExampleCatalogEntry = {
  dir: string;       // examples/<dir> in this workspace
  title: string;
  description: string;
...
const RUN_MAIN = 'synor update main';
const RUN_MAIN_PY = 'synor update main.py';
...
export const EXAMPLE_CATALOG_GROUPS: ExampleCatalogGroup[] = [
  ...
  },
  {
    ...
    entries: [
      { dir: 'local_note_catalog', docs: 'local-note-catalog', title: 'Local Note Catalog', description: 'Catalog Markdown notes as JSON using only local files, Python standard-library code, and the local Synor engine.', run: RUN_MAIN_PY },
      ...
      { dir: 'conversation_to_knowledge', docs: 'podcast-to-knowledge-graph', title: 'Podcasts → Knowledge Graph', description: 'Turn YouTube podcasts into a SurrealDB knowledge graph: diarized transcription, two-step LLM extraction, and embedding-based entity resolution.', run: 'synor update conv_knowledge.app' },
      ...
      { dir: 'csv_to_kafka', docs: 'csv-to-kafka', title: 'CSV → Kafka', description: 'Watch a folder of CSV files and publish each row as a JSON message to a Kafka topic: declarative target states, only-changed-rows produces, and live mode.', run: 'synor update -L main.py' },
  ...
  },
  {
    ...
    entries: [
      ...
      { dir: 'postgres_source', docs: 'postgres-source', title: 'Postgres as a Source', description: 'Use an existing PostgreSQL table as a Synor source: derive fields, embed, and store the results back.' },
  ...
  },
  {
    ...
    entries: [
      ...
      { dir: 'kafka_to_lancedb', docs: 'kafka-to-lancedb', title: 'Kafka → LanceDB', description: 'Consume JSON messages from a Kafka topic and dispatch them to two LanceDB tables by message shape.', run: 'synor update -L main.py' },
  ...
  {
    title: 'Rust',
    blurb: 'The same declarative flows using the Synor Rust API.',
    entries: [
      { dir: 'rust', title: 'Rust Examples', description: 'Rust ports of many of the examples above — the same declarative flows using the Synor Rust API.', run: 'cd rust/<example> && follow its README; common index command: cargo run -- index' },
    ],
  },
];
