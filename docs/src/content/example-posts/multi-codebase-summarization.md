...
For example, for each synor example project, we can have an auto-one-pager like this:
...
When your source data is updated, or your processing logic is changed (for example, switching to a different model, updating your LLM extraction logic), Synor performs smart incremental processing that only reprocesses the minimum. And it keeps your wikis always up to date in production.
...
1. Install Synor and dependencies:
    ...
    ```bash
    pip install 'synor>=0.1.0a1' instructor litellm pydantic
    ```
...
async def process_project(
    ...
    file_infos = await syn.map(extract_file_info, files)
...
```
**Concurrent processing with async** — By using `syn.map()`, all file extractions run concurrently while keeping Synor function calls visible to the pipeline. This is significantly faster than sequential processing, especially when making LLM API calls.
...
- **Components** — Lists classes and functions with descriptions (★ marks Synor functions)
...
3. **Async concurrent processing** with `syn.map()`
