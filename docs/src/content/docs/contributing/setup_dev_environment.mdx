...
description: >
  ...
  `maturin develop` to get a local Synor checkout ready for development.
meta:
  time: ~15 minutes
  requires: Rust + uv
---
Follow the steps below to get Synor built on the latest codebase locally - if you are making changes to Synor functionality and want to test it out.
...
- Build the library. Run at the root of synor directory:
...
To run examples during development, you need to use the local editable version of synor.
`.env.lib_debug` provides a `synor-dev-run` function to make it more convenient.

```sh
# Navigate to an example directory
cd examples/text_embedding

# Run with your local synor changes
synor-dev-run synor update main
```

The `synor-dev-run` function runs `uv run --with-editable $SYNOR_DEV_ROOT`, which ensures the example uses your locally built synor package instead of the published version. The `SYNOR_DEV_ROOT` variable is automatically set to the repo root when you source `.env.lib_debug`.
...
- Run cargo tests via:
  ...
  `./dev/run_cargo_test.sh -p synor --lib`
