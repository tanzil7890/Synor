...
- **Unannotated functions don't break the chain.** If `f1()` → `f2()` → `f3()`, and `f1` and `f3` are decorated with `@syn.fn` but `f2` is not, a logic change in `f3` still invalidates `f1`'s memo.
