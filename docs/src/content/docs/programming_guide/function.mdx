---
title: Declaring *functions* with @syn.fn
description: >
  The @syn.fn decorator makes Python functions participate in Synor's change
...
`@syn.fn` preserves the sync/async nature of the underlying function. Decorating a sync function yields a sync function; decorating an async function yields an async function.

## How to think about `@syn.fn`
...
**Logic changes** — the function's source code, [`deps`](#deps) values, and explicit [`version`](#version) bumps. Tracked by `@syn.fn`. Logic fingerprints **propagate transitively** up the call chain — but only through `@syn.fn` boundaries. If `foo` (memoized) calls `bar` (also `@syn.fn`-decorated, with or without `memo=True`), and `bar`'s logic changes, `foo`'s memo is invalidated. A bare Python helper that isn't decorated is invisible to change detection: editing it will not invalidate `foo`. This is why `@syn.fn` matters for **any** function in the call chain, not just memoized ones — see [Common patterns](#common-patterns).

**Input changes** — the function's arguments. Tracked by `@syn.fn`. When you call a function with different arguments, the fingerprints change. Input fingerprints do not propagate transitively.

**Context changes** — [context values](./context) with [`detect_change=True`](./context#change-detection). Tracked by [`use_context()`](./context#retrieving-values) at the call site — independent of `@syn.fn`. Context reads propagate transitively: if a memoized `foo` calls `bar` and `bar` reads a change-detected context value, then changing that value invalidates `foo`'s memo too — even though `foo` itself never called `use_context()`. What matters is whether the key was read *anywhere* during the memoized call, not where in the call chain.
...
- **Cannot mount child components** (`syn.mount(...)` / `syn.use_mount(...)` inside the body). Mounting is a side effect the cache cannot replay; Synor raises an error if a memoized function attempts it. Either drop `memo=True`, or restructure so the mount happens in a non-memoized caller.
:::

#### Cache-hit semantics

When a memoized function's cache hits, its **body does not run** — and neither do any nested `@syn.fn` calls inside it. The cached output is replayed directly: the previous return value is returned, and any target states the function declared on its previous run are carried over.
...
Three parameters on `@syn.fn` let you customize how logic changes are detected:
...
- **`"full"` (default):** Track this function's logic AND all transitively called `@syn.fn` functions' logic. A change anywhere in the call chain invalidates dependent memos.
...
@syn.fn(memo=True, deps=SYSTEM_PROMPT)
...
@syn.fn(memo=True, deps={"prompt": SYSTEM_PROMPT, "model": MODEL})
def summarize(text: str) -> str:
...
:::tip[`@syn.fn` on non-memoized helpers]
You can — and often should — decorate helpers with `@syn.fn` even without `memo=True`. The decorator's job is to make the function's logic *visible* to the change detection system. A bare helper is invisible: editing it will not invalidate any memoized caller, leading to silently stale results. Add `@syn.fn` so its fingerprint propagates; only add `memo=True` if caching its return value is worth it.
:::

These parameters can be set on any `@syn.fn` function — not just memoized ones.
...
async def process_file(file: FileLike) -> list[Chunk]:
    # Any change here or in called @syn.fn functions invalidates dependent memos
...
```python
@syn.fn(logic_tracking="self", version=3)
...
:::note
[Context changes](./context#change-detection) are independent of `@syn.fn` and `logic_tracking`. Even with `logic_tracking=None`, a change in a change-detected context value still invalidates dependent memos, because context tracking is done by `use_context()`, not by the decorator.
:::
...
1. **Logic** — did the function's source code change? Did any `@syn.fn` it transitively calls change? Was a `version` bumped? Was a `deps` value edited?
...
3. **Context** — did any [`detect_change=True`](./context#change-detection) context value read during the previous invocation change? Remember this propagates transitively through nested `@syn.fn` calls.
...
- A logic change in a helper that is **not** decorated with `@syn.fn`. Add `@syn.fn` to it (no `memo=` needed) so its logic participates in propagation — see [Common patterns](#common-patterns).
...
For per-function overrides — excluding an argument from the memo key, or transforming it just for this function — pass `memo_key={...}` on `@syn.fn` / `@syn.fn.as_async`; see [Override at the call site with `memo_key=`](../advanced_topics/memoization_keys#override-at-the-call-site-with-memo_key).
...
Use `@syn.fn.as_async` when you need an **async** interface for a function that has a sync underlying implementation. This is useful for compute-intensive leaf functions, and is required for features like [batching](#batching) and [runner](#runner).
...
`@syn.fn.as_async` is equivalent to wrapping the function in `asyncio.to_thread()` — the sync function runs on a thread pool and doesn't block the event loop.

You can also call any `@syn.fn`-decorated function asynchronously via the `.as_async()` method, without changing its primary signature:
...
Batching requires an async interface. If the underlying function is sync, use `@syn.fn.as_async(batching=True)`. If the underlying function is already `async def`, `@syn.fn(batching=True)` works directly.
...
```python
@syn.fn.as_async(batching=True, max_batch_size=32)
...
```python
@syn.fn(batching=True, max_batch_size=64)
...
Like batching, a runner requires an async interface. If the underlying function is sync, use `@syn.fn.as_async(runner=...)` to make it async. If the underlying function is already `async def`, `@syn.fn(runner=...)` works directly.
...
```python
@syn.fn.as_async(batching=True, max_batch_size=16, runner=syn.GPU)
