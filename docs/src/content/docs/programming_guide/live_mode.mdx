...
The `live` flag propagates top-down through the component tree — both `syn.mount()` and `syn.use_mount()` inherit `live` from the parent, so children are live when the app is live.
...
async def app_main() -> None:
    ...
    await syn.mount_each(process_message, items)
