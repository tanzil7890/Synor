...
Component paths are hierarchical and form a tree structure. You specify child paths using `syn.component_subpath()` with stable identifiers like string literals, file names, row keys, or entity IDs:

```python
syn.component_subpath(filename)           # e.g., syn.component_subpath("hello.pdf")
syn.component_subpath("user", user_id)    # e.g., syn.component_subpath("user", 12345)
```
...
await syn.mount(syn.component_subpath(syn.Symbol("process_file")), process_file, file, target)
```
...
- **The same function is mounted more than once** — auto-derived paths would be identical, so each call needs a distinct path (e.g., `syn.component_subpath("session", youtube_id)`)
...
```python
handle = await syn.mount(process_file, file, target)
```

With an explicit subpath, for example when mounting multiple components of the same function:

```python
handle = await syn.mount(
    syn.component_subpath("process", filename),
    process_file,
    file,
    target,
)
...
```python
table = await syn.use_mount(setup_table, table_name="docs")
```
...
In the static case this is just the keyed-loop form of [`mount()`](#mount), using the stable ownership model shown in [Core Concepts](./core_concepts#a-processing-component-owns-a-work-unit). The snippet above is equivalent to looping over the items and calling `mount(syn.component_subpath(syn.Symbol("process_file"), key), process_file, value, target)` for each one. The per-item `key` is what keeps each component's path stable across runs, so the engine matches, updates, and cleans up each item individually.

You can provide an explicit subpath as the first argument:

```python
await syn.mount_each(syn.component_subpath("files"), process_file, files.items(), target)
```
...
dir_target = await syn.mount_target(localfs.dir_target(outdir))
```
...
with syn.component_subpath("process"):
    for f in files:
        await syn.mount(
            syn.component_subpath(str(f.relative_path)),
            process_file,
            f,
            target,
        )
...
- **Coarse**: one component for "all files" (`syn.component_subpath("process")`)
- **Medium**: one component per file (`syn.component_subpath("process", file_path)`)
- **Fine**: one component per chunk (`syn.component_subpath("process", file_path, chunk_id)`)
