...
- Each call to `spawn()` or `call()` declares a **child processing component** at a child path. Sugar APIs like `spawn_each()` and `attach_target()` also create child components.
...
├── "setup"                    ← ensure_dir_target component
└── "process"
