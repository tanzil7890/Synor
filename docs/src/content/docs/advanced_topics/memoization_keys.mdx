...
The two approaches above are **type-level**: every memoized function sees the same fingerprint for a given object. To customize fingerprinting **only for a specific function**, pass `memo_key=` to `@syn.fn` (or `@syn.fn.as_async`). It maps parameter names to either a callable (transform the value before fingerprinting) or `None` (exclude the parameter entirely).

```python
@syn.fn(memo=True, memo_key={"entry": lambda e: (e.name, e.version), "extra": None})
...
1. **First run** (no previous state): `prev_state` is `syn.NON_EXISTENCE`. Use `syn.is_non_existence(prev_state)` to detect this.
...
class LocalFile:
    ...
    def __synor_memo_state__(self, prev_state: tuple[int, str] | syn.NonExistenceType) -> syn.MemoStateOutcome:
...
def path_state(p: Path, prev_state: tuple[int, int] | syn.NonExistenceType) -> syn.MemoStateOutcome:
    st = p.stat()
    new_state = (st.st_mtime_ns, st.st_size)
    memo_valid = not syn.is_non_existence(prev_state) and new_state == prev_state
...
- **In a sync Synor function**: if no event loop is running, Synor uses `asyncio.run()`. If a loop is already running, it raises an error — switch to an async function or use `@syn.fn.as_async`.
...
class S3Object:
    ...
    async def __synor_memo_state__(self, prev_state: str | syn.NonExistenceType) -> syn.MemoStateOutcome:
        etag = await self._head_object()
        memo_valid = not syn.is_non_existence(prev_state) and etag == prev_state
