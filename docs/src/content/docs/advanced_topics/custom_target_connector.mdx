...
child_provider = syn.declare_target_state_with_child(parent_target_state)
...
2. **Declaration**: During execution, user code calls `provider.target_state(key, spec)` to create `TargetState` objects, then `declare_target_state()` to declare them. Synor collects all declared target states.
...
class _RowHandler(syn.TargetHandler[_RowSpec, _RowTrackingRecord]):
    ...
    def reconcile(
        self,
        key: syn.StableKey,
        desired_target_state: _RowSpec | syn.NonExistenceType,
...
def declare_dir_target(path: pathlib.Path) -> DirTarget:
    ...
    child_provider = syn.declare_target_state_with_child(parent_ts)
...
class _FileHandler(syn.TargetHandler[_FileContent, _FileTrackingRecord]):
    ...
    def reconcile(
        self,
        key: syn.StableKey,
        desired_target_state: _FileContent | syn.NonExistenceType,
