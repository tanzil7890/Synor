...
description: >
  Connect Synor's declarative target-state system to external systems.
...
:::tip[Start Simple]
For simple use cases where you just need to write data to an external system without sophisticated change tracking, consider using a regular function with memoization instead. Target states providers are most valuable when you need Synor to track and clean up target states automatically.
:::

## Key data types

This section introduces the key data types. Each is marked as either **you implement** or **Synor provides** to clarify responsibilities.
...
Annotate the `prev_possible_records` parameter with `Collection[YourTrackingRecord]` so Synor can properly reconstruct stored tracking records during deserialization. See [Serialization](../programming_guide/serialization) for details on supported types.
:::
...
- Are **serializable**: Must be persistable (typically a NamedTuple or dataclass). Dataclasses and NamedTuples are serialized with msgspec automatically. For types requiring pickle, use `@syn.serialize_by_pickle`.
...
```python
# Sync sink
sink = syn.TargetActionSink.from_fn(apply_actions)

# Async sink
sink = syn.TargetActionSink.from_async_fn(apply_actions_async)
```
...
) -> list[syn.ChildTargetDef[_ChildHandler] | None]:
    outputs = []
    for action in actions:
        if action.content is None:
            ...
            if action.is_directory:
                outputs.append(syn.ChildTargetDef(handler=_ChildHandler(action.path)))
            else:
                outputs.append(None)
    return outputs
```

#### Declare operational guarantees

Every sink exposes a versioned, machine-readable capability contract. Existing
connectors that do not declare one report `"unknown"` rather than receiving an
...
sink = syn.TargetActionSink.from_async_fn(
    apply_actions_async,
    capabilities=capabilities,
)

assert sink.capabilities.to_dict()["schema_version"] == 1
```

The contract distinguishes these concerns:

| Field | Meaning |
|---|---|
| `batch_atomicity` | `none`, one atomic unit per action (`per_action`), or one atomic unit for the complete `apply` call (`per_apply`) |
| `idempotent_replay` | Whether replaying an acknowledged action preserves the same external state |
| `segmented_replay_safe` | Whether Synor may split one component's actions across multiple `apply` calls and safely replay an already-applied prefix |
| `apply_ordering` | Whether actions become visible in input order or in an unspecified order |
| `cancellation_safe` | Whether interruption leaves a state that a later reconciliation can safely retry |
| `completion_verification` | `unverified` callback completion, an external provider acknowledgement (`acknowledged`), or a postcondition query (`query_verified`) |
| `max_batch_actions` | Maximum actions accepted by one `apply` call |
| `max_batch_bytes` | Best-effort in-memory byte limit accepted by one `apply` call |

The engine uses 4,096 actions and approximately 8 MiB as internal packing
thresholds when coalescing independently submitted work. They are not hard
limits on one component's action set. Legacy sinks and sinks with an `unknown`
or `unsupported` segmentation capability therefore keep the compatibility
contract of one `apply` call, even when that call is larger. Set
`max_batch_actions` or `max_batch_bytes` to publish a hard per-call connector
limit; Synor preflights that declared limit before external side effects.

Set `segmented_replay_safe="supported"` only when failure injection proves that
the sink can safely replay a prefix that a previous attempt already applied.
This claim requires `idempotent_replay="supported"`. Synor may then deliver the
component's actions in ordered, bounded segments. Earlier segments can become
externally visible before a later segment fails, so segmentation weakens
external all-at-once visibility even when `batch_atomicity="per_apply"`.
Synor commits the component's internal tracking state only after every segment
succeeds; a later failure leaves the component failed and eligible for a full
retry. A single action larger than a connector's declared byte limit is rejected
before the first sink call. A single action larger only than the engine's
internal packing threshold is delivered alone.

`sink.queue_stats` provides point-in-time pressure telemetry: ongoing and queued
batches, queued and in-flight inputs, estimated in-flight bytes, and the number
of submitters currently waiting for capacity. These values are operational
signals, not cumulative counters.

Only claim a field after a connector-level failure-injection test exercises the
relevant boundary. A fully certified sink covers success, retry after an
acknowledgement loss, failure partway through a batch, cancellation during I/O,
and recovery from the resulting external state. Partial certification is
allowed, but untested fields stay `"unknown"` or `"unsupported"` instead of
assuming the strongest deployment configuration.

The reusable test runner is available without a pytest dependency:
...
report = await certify_target_sink(
    TargetSinkCertificationScenario(
        name="my-connector-row-sink",
        capabilities=sink.capabilities,
        actions=actions,
        reset=reset_external_state,
        apply=apply_through_sink,
        snapshot=read_external_state,
        expected_final_snapshot=expected_state,
        apply_with_failure_after=apply_with_injected_failure,
        cancel_apply=cancel_during_external_io,
        observed_order=read_observed_order,
        expected_order=expected_order,
        observed_batches=read_observed_batches,
        action_size_bytes=estimate_action_size,
        completion_evidence=read_completion_evidence,
        apply_with_completion_failure=apply_with_rejected_acknowledgement,
    )
)
```

The runner requires evidence hooks only for the guarantees actually claimed,
and treats a missing hook as a certification failure. Synor's built-in sink
inventory is checked in at `dev/target-sink-certification.json`; CI rejects a
new positive claim unless that manifest names its failure-injection test.
...
class TargetReconcileOutput(Generic[ActionT, TrackingRecordT, OptChildHandlerT], NamedTuple):
    ...
    tracking_record: TrackingRecordT | AbsentType  # What to remember
...
### TargetStateProvider *(Synor provides)*

A `TargetStateProvider` is a factory that creates `TargetState` objects. You don't implement this class — Synor gives you one when you register a handler or declare a target state with children.

```python
# You get a provider from registration
provider = syn.register_root_target_states_provider("my.target", handler)

# Or from declaring a parent target state
child_provider = syn.ensure_target_state_with_child(parent_target_state)
...
1. **Registration**: You define a `TargetHandler` and call `register_root_target_states_provider()`. Synor returns a `TargetStateProvider` — a factory for creating target states associated with your handler.

2. **Declaration**: During execution, user code calls `provider.target_state(key, spec)` to create `TargetState` objects, then `ensure_target_state()` to declare them. Synor collects all declared target states.

3. **Reconciliation**: When the processing unit finishes, Synor calls your handler's `reconcile()` method for each target state. For declared target states, `desired_target_state` contains the spec; for previously declared but now missing states, `desired_target_state` is `ABSENT` (triggering cleanup). Your `reconcile()` compares the desired state with previous records and returns `TargetReconcileOutput` if an action is needed, or `None` if no changes are required.

4. **Action Execution**: Synor batches actions by their `TargetActionSink` and executes them. The sink applies changes to the external system (database writes, file operations, API calls, etc.).

5. **Tracking Persistence**: After successful execution, Synor persists the new tracking records. On the next run, these become the `prev_possible_records` for change detection.

:::note[Multiple Previous States]
Due to interrupted updates, `prev_possible_records` may contain multiple records. Synor tracks all possible states until a successful update confirms the current state. Your reconciliation logic should handle this by generating actions that work correctly regardless of which previous state is actual.
:::
...
```python
class _RowHandler(syn.TargetHandler[_RowSpec, _RowTrackingRecord]):
    ...
    async def _apply_actions(
        self, context_provider: syn.ContextProvider, actions: Sequence[_RowAction]
    ) -> None:
    ...
    def reconcile(
        self,
        key: syn.StableKey,
        desired_target_state: _RowSpec | syn.AbsentType,
        prev_possible_records: Collection[_RowTrackingRecord],
        prev_may_be_missing: bool,
        /,
    ) -> syn.TargetReconcileOutput[_RowAction, _RowTrackingRecord] | None:
        # Handle deletion
...
```python
_row_provider = syn.register_root_target_states_provider(
...
class TableTarget:
    ...
    def __init__(self, provider: syn.TargetStateProvider[_RowSpec, None]):
        self._provider = provider

    def ensure_row(self, *, row: dict[str, Any], key: tuple[str, ...]) -> None:
...
1. **Declaration**: Call `ensure_target_state_with_child(parent_ts)` — returns an **unresolved** child provider immediately
2. **Resolution**: When the parent reconciles and its sink executes, the sink returns `ChildTargetDef(handler=...)`. Synor resolves the child provider with this handler.
...
When a container target undergoes certain changes, the child target states may be affected. The `child_invalidation` field in `TargetReconcileOutput` lets you signal this to Synor:

- **`"destructive"`** — The container change destroys all existing children (e.g., a primary key change that requires dropping and recreating a table). Synor will ignore all previous tracking records for children under this container and treat them as new.

- **`"lossy"`** — The container change may cause data loss for existing children (e.g., a schema change that removes columns). Synor will force an upsert for all children by setting `prev_may_be_missing=True`, even if their data appears unchanged.
...
```python
class _DirHandler(syn.TargetHandler[_DirSpec, _DirTrackingRecord, _EntryHandler]):
...
class _EntryHandler(syn.TargetHandler[_EntrySpec, _EntryTrackingRecord]):
...
def _apply_dir_actions(
    ...
    for action in actions:
        if action.should_delete:
            ...
            outputs.append(syn.ChildTargetDef(handler=_EntryHandler(action.path)))
    return outputs
```

### Step 3: Create user-facing API

The user-facing API uses `ensure_target_state_with_child()` and exposes methods for declaring children:

```python
class DirTarget:
    """User-facing API for declaring files in a directory."""

    def __init__(self, provider: syn.TargetStateProvider[_EntrySpec, None]):
        self._provider = provider

    def ensure_file(self, filename: str, content: bytes) -> None:
        spec = _EntrySpec(content=content)
        target_state = cast(
            syn.TargetState[None],
...
@syn.task
def ensure_dir_target(path: pathlib.Path) -> DirTarget:
    ...
    child_provider = syn.ensure_target_state_with_child(parent_ts)
...
class _VectorIndexHandler:
    ...
    async def _apply_actions(
        self, context_provider: syn.ContextProvider, actions: Sequence[_VectorIndexAction]
    ) -> None:
...
def _apply_actions(
    context_provider: syn.ContextProvider, actions: Sequence[_MyAction]
) -> None:
    ...
...
class _MyHandler(syn.TargetHandler[...]):
    def reconcile(self, ...):
        return syn.TargetReconcileOutput(
            action=...,
...
# Action execution
def _apply_actions(context_provider: syn.ContextProvider, actions: Sequence[_FileAction]) -> None:
    for action in actions:
        if action.content is None:
            action.path.unlink(missing_ok=True)
        else:
...
_file_sink = syn.TargetActionSink[_FileAction, None].from_fn(_apply_actions)


# Handler
class _FileHandler(syn.TargetHandler[_FileContent, _FileTrackingRecord]):
    ...
    def reconcile(
        self,
        key: syn.StableKey,
        desired_target_state: _FileContent | syn.AbsentType,
        prev_possible_records: Collection[_FileTrackingRecord],
        prev_may_be_missing: bool,
        /,
    ) -> syn.TargetReconcileOutput[_FileAction, _FileTrackingRecord] | None:
        path = self._base_path / key
...
- `synor/connectors/localfs/target.py` — File system targets
- `synor/connectors/postgres/target.py` — PostgreSQL tables and rows
