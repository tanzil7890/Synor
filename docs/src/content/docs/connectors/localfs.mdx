...
**Returns:** A pending `DirTarget`. Use `await syn.mount_target(...)` or the convenience wrapper `await localfs.mount_dir_target(path)` to resolve.
...
async def app_main() -> None:
    ...
    await syn.mount_each(process_file, localfs.walk_dir(SOURCE_DIR, recursive=True).items(), target)
