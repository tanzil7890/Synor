...
When `live_stream` is provided, `items()` returns a [`LiveMapView`](../advanced_topics/live_component#livemapfeed-and-livemapview) instead of a plain `AsyncIterable`. Combined with [`spawn_each()`](../programming_guide/processing_component#spawn_each), this enables automatic incremental bucket watching — newly created, updated, and deleted objects are processed without a full rescan.
...
Because this consumer comes from `kafka.create_consumer()`, binding it transfers ownership to the single-use topic stream. The stream drains pending work, unsubscribes, and closes the consumer whenever watching ends.
...
async def process_file(file: FileLike[str], target: localfs.DirTarget) -> None:
    text = await file.read_text()
    target.ensure_file(filename=file.file_path.path.as_posix(), content=text)
