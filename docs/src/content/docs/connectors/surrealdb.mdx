...
SURREAL_DB: syn.ContextKey[surrealdb.ConnectionFactory] = syn.ContextKey("main_db")
...
The `surrealdb` connector provides target state APIs for writing records to normal tables and relation tables. Synor tracks what records should exist and automatically handles upserts and deletions.
...
- `managed_by` — Whether Synor manages the table lifecycle.
...
Declare a vector index on a field of the table. Synor tracks the index spec and automatically creates, recreates, or drops the index as needed.
