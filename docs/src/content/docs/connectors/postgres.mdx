...
description: >
  Read Postgres tables via PgTableSource and write with ensure_table_target() —
...
async def app_main(pool: asyncpg.Pool) -> None:
    ...
    async for product in source.fetch_rows():
        syn.spawn(
            syn.unit_path("product", product.product_id),
            process_product,
            product,
        )
```
...
The `postgres` connector provides target state APIs for writing rows to tables. With it, Synor tracks what rows should exist and automatically handles upserts and deletions.
...
PG_DB = syn.ContextKey[asyncpg.Pool]("my_db")
...
Declare a pgvector index on a vector column of the table. Synor tracks the index spec and automatically creates, recreates, or drops the index as needed.
...
Declare an arbitrary SQL command that Synor manages alongside the table. The setup SQL runs when the attachment is created or changed; the optional teardown SQL runs when the attachment is removed or before re-running setup on change.
...
U+0000 (NUL) is a valid Unicode codepoint, but Postgres cannot store it — neither in `text`-family columns nor inside strings in `jsonb` (the `\u0000` escape is rejected at parse time). Synor automatically strips U+0000 from strings before writing to Postgres, recursively for nested strings and dict keys in `jsonb` payloads. For example, `"Hello\0World"` is written as `"HelloWorld"`.
:::
...
PG_DB = syn.ContextKey[asyncpg.Pool]("main_db")
