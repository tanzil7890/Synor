...
The `doris` connector provides target state APIs for writing rows to tables. Synor tracks what rows should exist and automatically handles upserts and deletions via Doris stream load.
...
DORIS_DB = syn.ContextKey[doris.ManagedConnection]("my_doris")
...
```python
def DorisTableTarget.ensure_row(
    self,
    *,
    row: RowT,
) -> None
...
from synor.connectors.doris import DorisType
...
```python
from synor.connectors.doris import VectorIndexDef
...
```python
from synor.connectors.doris import InvertedIndexDef
