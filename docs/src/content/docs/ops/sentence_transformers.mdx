...
---
The `synor.ops.sentence_transformers` module provides integration with the [sentence-transformers](https://www.sbert.net/) library for text embeddings.
...
```bash
pip install synor[sentence_transformers]
```
...
table.ensure_row(row=CodeEmbedding(code="Hello, world!", embedding=embedding))
```
