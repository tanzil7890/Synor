...
---
The `synor.ops.entity_resolution` package resolves a set of raw entity names into a deduplicated canonical map. It finds near-duplicate names via embedding similarity (FAISS), then asks a pair-resolver (typically an LLM) to confirm matches and pick the better canonical name.
...
pip install synor[entity_resolution_llm]

# Core only (for custom resolver implementations)
pip install synor[entity_resolution]
```
...
async def resolve_my_entities(raw_entities: set[str]) -> dict[str, str | None]:
    result = await resolve_entities(
        ...
        resolve_pair=LlmPairResolver(model=syn.use_context(LLM_MODEL)),
    )
...
```python
from synor.ops.entity_resolution import ExistingCanonicalPolicy
...
```python
from synor.ops.entity_resolution import ResolutionEvent
...
Each unique `(entity, candidates)` pair's decision is persisted across runs via [`@syn.task(cache=True)`](../programming_guide/function). Changing the model or prompt invalidates the cache. No additional memoization wrapper is needed.
...
```python
from synor.ops.entity_resolution import PairResolver, PairDecision, CanonicalSide
