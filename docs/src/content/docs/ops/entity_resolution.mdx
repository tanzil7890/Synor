...
Each unique `(entity, candidates)` pair's decision is persisted across runs via [`@syn.fn(memo=True)`](../programming_guide/function). Changing the model or prompt invalidates the cache. No additional memoization wrapper is needed.
